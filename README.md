...
**Customer Identification:** The surrogate key for customers was initially created based on customer name alone, with the assumption that each unique name would correspond to a single key. However, this approach led to multiple records for a single customer when an order was split across different delivery addresses or included purchases for both business and personal purposes. To address this, a new surrogate key was implemented based on a unique combination of customer name, region, and province. This ensures that the customer key is assigned with sufficient granularity for accurate analysis based on region or segment, as using only the customer name without these additional criteria could lead to inaccurate analysis of customer segments or regions. 

**Handling Multiple Deliveries and Customers per Order:** There were cases where a single order was split for delivery to multiple addresses or involved multiple client segments. To resolve this, the assumption was made that each unique order, based on customer details and delivery information, would be treated as a separate entity for analysis. Each order with distinct delivery details is now treated as an independent transaction, with customer keys assigned based on combinations of order id, customer name, region, province, and segment.

**Customer Segmentation and Recurring Customers:** In analyzing customer behavior, the assumption is made that a single customer may have multiple purchasing purposes or operate across different regional branches. As such, customer segmentation is based on a combination of their name and regional attributes, allowing for differentiated tracking. Due to the absence of a clear business definition for identifying returning customers, the analysis did not include specific assumptions or criteria for recognizing repeat customers within the dataset. Instead, customer segmentation focused solely on differentiating customers based on name, region, and segment without considering return frequency.

**Sales Data Corrections:** During data review, certain discrepancies were identified in the SALES values within the fact table. These were corrected through recalculations to ensure consistency with expected values. Additionally, unreliable columns such as PROFIT and MARGIN were removed to ensure the analysis focused on valid data. In a real business scenario, these corrections would have been made in consultation with relevant teams to align with company standards and definitions. However, for this fictional project, no assumptions were made regarding the creation of fictitious data.
